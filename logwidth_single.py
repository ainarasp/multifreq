...
for k in os.listdir('./values/%03dGHz/' %(args.freq[0]/1e9)):
...
for i in range(len(file_list)):
    ...
    angle = beamparams[2] 
    rotated_angle = angle + 15*np.pi/180
    width_beam = sqrt((beamparams[0]*cos(rotated_angle))**2 + (beamparams[1]*sin(rotated_angle))**2) # this is in radians
    width_beam = width_beam/eh.RADPERUAS/1000
    width_beam = 1*width_beam
    major = beamparams[0]/eh.RADPERUAS/1000
    minor = beamparams[1]/eh.RADPERUAS/1000

    # Load computed values
    data = np.load('./values/%03dGHz/'%(args.freq[0]/1e9)+file_list[i])
    width = data['width']
    widths.append(width)
    peak = data['peaks']
    peaks.append(peak)
    ...
    beam_major.append(major)
    beam_minor.append(minor)
...
beam_angle = np.mean(beam_angle)
beam_major = np.mean(beam_major)
beam_minor = np.mean(beam_minor)
beam_width = np.mean(beam_width)
gaussian_zeros = np.mean(gaussian_zeros,axis=0)

np.savez('./values/%03dGHz/'%(args.freq[0]/1e9)+'NGC1052_avg_%03dGHz.npz'%(args.freq[0]/1e9),width=avg_width,std_width=std_width,xplt=x,
    ...
    angle=beam_angle,major=beam_major,minor=beam_minor,beam_width=beam_width,zeros=gaussian_zeros)

np.save('./values/%03dGHz/'%(args.freq[0]/1e9)+'NGC1052_frame_%03dGHz.npz'%(args.freq[0]/1e9),frame)
np.save('./values/%03dGHz/'%(args.freq[0]/1e9)+'NGC1052_frame_nonblur_%03dGHz.npz'%(args.freq[0]/1e9),frame_nonblur)

plt.figure()
plt.imshow(frame,extent=[x[0],x[-1],x[-1],x[-0]])
plt.plot(x,contour_top,'r')
plt.plot(x,contour_btm,'r')
ell = mpl.patches.Ellipse([7.5,-4],beam_major,beam_minor,color='white',angle=-105-beam_angle*180/np.pi)
